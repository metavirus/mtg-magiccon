...
The catalog tranche is implemented locally but not published and no catalog migration has been applied to Supabase. The prepared schema separates immutable source captures, extracted observations, reviewed canonical products/variants/offers, presentation media, append-only availability (including `sold_out` and event-local day), and per-person shopping interests. Catalog purpose now distinguishes publishable event `inventory` from non-publishable historical `reference` material; the promotion RPC rejects reference plans and the production read view independently excludes them. The production read path otherwise fails closed and reads only reviewed current offers; interest writes require exact owner binding and readback.
