...
Receipt originals, QR codes, and transfer proof now live in the private `private-receipt-artifacts` Supabase Storage bucket rather than the public app bundle. After an authenticated refresh, the app automatically saves the owner-scoped receipt index and every permitted receipt artifact—including all originals—to the device whether or not Wallet has been opened. Cache keys include the owner id, no token is persisted, explicit sign-out clears the private artifact cache and offline identity, and the account menu exposes the actual private-proof completion count instead of treating a partial download as ready.
...
The service worker auto-updates the review build and precaches the static shell plus bounded packaged Map and artist assets. Authenticated device hydration separately saves the complete user-visible production read model: companion roster, notes, selections and Plan overlays, Activity, mentions, monitoring findings/concepts, Info knowledge, ticketed availability, Trip flights, the Wallet receipt index and all permitted proof artifacts, the active Atlanta artist catalog and signing interests, all active artist/card images, and current catalog presentation media. Private/live JSON and Wallet proof remain excluded from the public precache; they enter only the owner-scoped device cache after authentication.

Local 390px automation proves a cold offline reopen of the shell, Wallet, and Artists with all four packaged portraits. Contract tests require automatic hydration of all receipt artifacts—not a QR-only subset—and the active artist/card media set; private proof blobs reopen from an owner-scoped cache, and installed standalone launches strip fixture/QA parameters instead of silently entering preview mode. A previously authenticated offline launch may use a token-free cached identity only to select read-only owner data; writes still require a live Supabase session and network. Hidden QA/reference catalogs, unrelated inactive card corpora, first-time authentication, and external web destinations are not part of the production device pack. Physical iPhone acceptance remains open until the published build reports the proof pack complete and survives a cold airplane-mode reopen across the production surfaces.
...
2. On a physical iPhone, refresh/install the corrected production build, wait for the account menu to report the private proof pack complete, then prove Google OAuth persistence plus installed-PWA cold airplane-mode reopen across Wallet originals, Artists, Map, Trip, Plan, and Info. Record any unavailable production object before treating the pack as accepted.
