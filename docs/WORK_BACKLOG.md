...
- [x] **v1.5:** Replace fake Black Lotus proof with functional proof routing. Wallet now opens the real Leap order links and original email reference, and records that the Black Lotus QR has been captured from Gmail. Direct QR display waits for authenticated Supabase Storage rather than a public static asset.
...
- [ ] **v1.5/trust:** Design the private Storage artifact workflow for cross-device QR, receipt PNG/PDF, and showable proof retrieval. The current public-preview QR bundle is an owner-approved shortcut, not the final storage model.
