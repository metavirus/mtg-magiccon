...
- [x] **v1.5:** Replace fake Black Lotus proof with a functional private-proof placeholder. Wallet now opens the real Leap order links and original email reference, and supports adding the QR image into local browser storage for review/use without publishing the live QR into the public preview.
...
- [ ] **v1.5/trust:** Design the private Storage artifact workflow for cross-device QR, receipt PNG/PDF, and showable proof retrieval. Public GitHub Pages fixtures must not contain live QR codes, reservation numbers, or full private receipt artifacts.
