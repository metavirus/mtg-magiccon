...
| [ ] | `MAP-LATER-06` | **Planned when published:** Design the Official Show Store close view once its final location and merchandise data exist. | confirmed 2026 venue/store artifacts | parked Map/Store integration | Preserve Map context while offering compact Wallet status, flagged merchandise, the full item list, and deliberate navigation to Wallet/Store; do not invent useful interior geometry where the store is only a queue and counter. |
