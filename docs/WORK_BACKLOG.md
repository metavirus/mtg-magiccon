...
- [x] **v1.5/monitoring:** Consequence-aware Kavi action execution, lifecycle audit, idempotent reviewed-Activity publication, RLS/privacy proof, cloud surveyor activation, and the real Magic Play action mapping are live. Commit `f0fe3f1`, CI `32520980614`, Pages `32520980683`, public verification, deployed desktop/mobile Activity inspection, and successful surveyor runs `32521125551` / `32521224614` are proven. Fingerprint `bc7e5a…` remained one row while `occurrence_count` reached `2`; the obsolete prototype `fdb94…` row was removed. The real finding remains `needs_review` for Kavi's explicit decision; agents must not decide it. No baseline was accepted.
