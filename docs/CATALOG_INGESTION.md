...
Rejected items remain in retained review history and are excluded from promotion. The ready result is sorted and transaction-shaped. `public.promote_catalog_batch(jsonb)` is a roster-bound, atomic, idempotent RPC: fixture, pending, malformed, or incomplete plans fail closed; offers remain unpublished until matching approved evidence and exact presentation media exist; same-batch replay returns the stored receipt while changed input under the same key fails. The client accepts only the exact `{status,batch_key,catalog_id,source_capture_id,promotions,promoted_count,retained_review_count}` readback, refreshes the canonical catalog projection, and only then reports success. Focused planner/client coverage passes, as does the live `pnpm validate:catalog-db` suite with 71 schema, RLS, grant, RPC, and value-kind assertions. Canonical migration `20260830155905_canonical_event_catalogs` is live as of 2026-08-30. No placeholder or real catalog content has been promoted: the hosted catalog tables and current-offer projection remain empty until a reviewed Atlanta catalog arrives.

The private `private-catalog-artifacts` bucket is provisioned through the Storage API when the first real intake needs an upload; the migration already installs its roster-bound read and operator-only immutable-insert policies. Do not create or populate that bucket merely for placeholder data.
