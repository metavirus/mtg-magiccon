...
**Remote-content-in-original trap:** Raw receipt HTML may depend on remote images that disappear offline, but manufacturing separate QR crops and page fragments is not the remedy. The primary proof must be one self-contained Gmail-looking artifact with required remote assets baked into it; if a QR appears in the source message, it should remain visible inside that proof. Keep raw HTML only as an archival fallback, and do not block structured receipt intake merely because the source has no separately extractable QR. `scripts/lib/private_gmail_intake.test.ts` guards against restoring the separate-QR requirement.

**Receipt proof overproduction and microscopic-proof trap:** A saved Gmail body can look unauthentic, but turning one email into extracted QR crops and a large upload manifest is also a failure. The user requirement is the simple Ctrl-P experience: a self-contained, scrollable Gmail-looking proof that preserves account, subject, sender, recipient, timestamp, and the complete message. Prefer one full-height image only when it remains readable at phone width. If fitting the full image makes the receipt microscopic, use a small ordered set of letter-sized page images from that same print render; `selectReceiptArtifactsForDisplay()` prefers the readable page set and hides the redundant full-height fallback. PDF is also acceptable when it is simpler. Never use a visible desktop file chooser as an agent ingestion transport. `scripts/render_gmail_print_artifact.mjs` and receipt artifact contract tests guard this lane.

**Companion-code privacy and coverage trap:** A Wizards Companion join code may arrive in a private receipt, but it is public event data. Never make Calendar visibility depend on the current viewer's receipt, purchaser identity, or personal purchase state. Intake publishes exact event/code pairs to `ticketed_play_public_companion_codes`; that table exposes only `event_id`, `companion_code`, and freshness, while receipt originals, QR artifacts, attendee identity, and purchase evidence remain private. Creating the public projection is not complete if only newly ingested receipts are seeded: reconcile every previously confirmed event/code pair into the projection, then verify public readback for each pair. Future intake must keep its per-event public readback check. `src/lib/purchaseUiContract.test.ts` guards the separation and the confirmed historical coverage.
