...
**Receipt proof overproduction trap:** A saved Gmail body can look unauthentic, but turning one email into a PDF, many page images, extracted QR crops, and a large upload manifest is also a failure. The user requirement is the simple Ctrl-P experience: one self-contained, scrollable Gmail-looking proof that preserves account, subject, sender, recipient, timestamp, and the complete message. Prefer one full-height proof image because it renders reliably in the existing offline modal; PDF is also acceptable when it is simpler. Raw HTML and legacy page slices may remain archival fallbacks, but `selectReceiptArtifactsForDisplay()` must prefer the single primary proof. Never use a visible desktop file chooser as an agent ingestion transport. `scripts/render_gmail_print_artifact.mjs` and its contract test guard this lane.
