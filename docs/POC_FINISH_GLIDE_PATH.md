...
- Tier 1 public preview: use `pnpm build` for one tightly bounded UI/routing fix or `pnpm check:ship` for broader/multi-surface work, visually inspect the affected viewport, commit/push `main`, wait for the GitHub Actions Pages deploy, then run `pnpm verify:public` and check the deployed flow.
