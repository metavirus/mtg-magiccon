...
## Private normalized-source consequence adapter

The executable adapter accepts one already-normalized private-source payload on stdin. It never queries or mutates Gmail. Any upstream caller that supplies a Gmail-derived payload is a separate capability and must be named and authorized separately; this repository does not provide Gmail discovery or connector automation.

The tracked `Manual receipt payload publisher (recovery)` workflow is a human-dispatched receipt-only wrapper around this adapter. It refuses non-receipt payloads, checks the expected source message id, and publishes the reviewed payload through the canonical server-secret lane. It is not a heartbeat, monitor, mailbox watcher, or automatic ingestion job. Its successful result is `payload_published` with `completion.status: verification_required`, because database and Storage readbacks do not prove that the showable original, shared authenticated download, or Wallet rendering is complete. Without an explicit passing proof-bundle/readability/operational-QR declaration, the same result must also say `presentation.status: not_certified`; a declared presentation pass still does not close the shared-download or Wallet-rendering checks. The guarded HOGFBX flight RPC remains an adapter capability but is not dispatchable through this receipt workflow.

The adapter planner returns `covered` only after source identity, stable Gmail message ID, canonical target identity, required facts, and all consequence guards are exact. Missing writer credentials, a noncanonical Supabase URL/key shape, an unknown attendee or event, incomplete flight legs, or any replacement ambiguity returns `not_covered` with a reason and performs no write. The manual receipt executor returns `payload_published` plus its incomplete-until-verified status; the separately authorized flight executor returns `applied`. Neither result includes original HTML or other private evidence in its compact summary.
...
- `receipt`: the manual publisher accepts only a Kavi-owned Gmail-derived normalized payload with archival source HTML and an exact attendee set. That archival object is not a complete showable proof bundle. A companion-only Ticketed Play order must name its attendee set explicitly; single-attendee shorthand remains limited to Kavi so inferred ownership cannot silently move a receipt. Every attendee must resolve to one active companion user, and every ticketed-play line must bind its attendee set and quantity without ambiguity. Ticketed-play receipts additionally require every line to bind to an exact `ticketed-<source id>` event and every event ID to read back from the canonical availability projection before any receipt or purchase-lock write. One shared order remains one receipt artifact; every bound attendee receives the corresponding permanent purchase locks and RLS-backed receipt access. Omitted receipt confidence defaults to `needs_review`, never `verified`.
...
Any explicitly authorized upstream caller must retain the adapter status for every normalized private candidate. `not_covered` is attention-required coverage debt, not a successful catch and not permission to advance a private-source baseline. `payload_published` is a manual receipt publication outcome, not a completion claim.
