...
- `receipt`: covered only for Kavi-owned Gmail messages with a complete artifact bundle and an exact attendee set that includes Kavi. Additional attendees are allowed only when they were explicitly confirmed, each resolves to one active companion user, and every ticketed-play line binds its attendee set and quantity without ambiguity. Ticketed-play receipts additionally require every line to bind to an exact `ticketed-<source id>` event and every event ID to read back from the canonical availability projection before any receipt or purchase-lock write. One shared order remains one receipt artifact; every bound attendee receives the corresponding permanent purchase locks and RLS-backed receipt access.
