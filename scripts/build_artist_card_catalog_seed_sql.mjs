...
export function buildArtistCardCatalogSeed(cardCsv, profileCsv) {
const cardRows = aggregatePrintingQuantities(parseCsv(cardCsv).filter(row => row.Artist && row.Name))
...
const seed = buildArtistCardCatalogSeed(readText(cardCsvPath), readText(profileCsvPath))
fs.writeFileSync(outputPath, seed.sql)
console.log(`Artist/card seed SQL written: ${path.relative(repoRoot, outputPath)}`)
console.log(`Artists in catalog: ${seed.artistCount}`)
