...
if (plan.kind === 'receipt') {
  ...
  const write = await client.from('wallet_receipts').upsert(receipt, { onConflict: 'owner_id,source_system,source_message_id' }).select('id,source_message_id,attendee_person_key').single()
  if (write.error) throw write.error
  const selections = plan.operation.eventIds.flatMap(eventId => [
    ['state', 'committed'],
    ['purchased', 'true'],
  ...
  const readback = await client.from('wallet_receipts').select('id,source_message_id,attendee_person_key').eq('owner_id', ownerId).eq('source_system', 'gmail').eq('source_message_id', plan.sourceMessageId).single()
  if (readback.error) throw readback.error
  const lockReadback = plan.operation.eventIds.length
    ? await client.from('user_selections').select('object_id,selection_key,selection_value').eq('owner_id', ownerId).in('object_id', plan.operation.eventIds.map(eventId => `explore-${eventId}`)).in('selection_key', ['state', 'purchased', 'purchase_locked'])
    : { data: [], error: null }
  if (lockReadback.error) throw lockReadback.error
  if ((lockReadback.data?.length ?? 0) !== plan.operation.eventIds.length * 3) throw new Error('Receipt applied without complete purchase-lock readback.')
  console.log(JSON.stringify({ status: 'applied', kind: 'receipt', sourceMessageId: plan.sourceMessageId, receiptId: readback.data.id, purchaseLockCount: plan.operation.eventIds.length }))
} else {
