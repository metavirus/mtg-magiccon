...
create policy "active_companions_select_group_selections"
  ...
  using (
    ...
    or (
      ...
      and user_selections.selection_key in ('state', 'purchased', 'purchase_locked')
      and exists (
        select 1
        from public.companion_members viewer
        where viewer.user_id = (select auth.uid()) and viewer.active
      )
      and exists (
        select 1
        from public.companion_members owner
        where owner.user_id = user_selections.owner_id and owner.active
      )
    )
    or (
      ...
      and exists (
        select 1
        from public.companion_members owner
        where owner.person_key = 'kavi' and owner.user_id = user_selections.owner_id and owner.active
      )
      and exists (
        select 1
        from public.companion_members viewer
...
comment on policy "active_companions_select_group_selections" on public.user_selections is
  'All active companions share event planning, purchase, and permanent-lock state. Juan additionally shares Kavi''s Prize Tix balance; all other selections remain owner-private.';
