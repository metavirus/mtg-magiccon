...
create policy "active_companions_select_group_selections"
  ...
  using (
    ...
    or (
      user_selections.object_kind = 'event'
      and user_selections.selection_key in ('purchased', 'purchase_locked')
      and exists (
        select 1 from public.companion_members viewer
        where viewer.user_id = (select auth.uid()) and viewer.active and viewer.person_key in ('kavi', 'chris')
      )
      and exists (
        select 1 from public.companion_members owner
        where owner.user_id = user_selections.owner_id and owner.active and owner.person_key in ('kavi', 'chris')
      )
    )
    or (
...
comment on policy "active_companions_select_group_selections" on public.user_selections is
  'Active companions share event planning state. Kavi and Chris additionally share purchase and permanent-lock state with each other. Juan shares Kavi''s Prize Tix balance; all other selections remain owner-private.';
