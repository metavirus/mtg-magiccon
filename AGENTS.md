...
Use the smallest capable lane. Small or cohesive work stays in root with zero subagents by default; never create a worker merely to demonstrate delegation. Anticipate context pressure before root is already overloaded: sustained duration is an early warning, not a timer or automatic dispatch rule, and heavy investigation, logs, trial-and-error, or a separate browser-proof campaign are stronger signals. When one substantial compartment is independently verifiable and separating it will protect user/product continuity, give it early to one fresh cohesive worker. Use parallel agents only for genuinely independent work when the wall-clock benefit exceeds coordination cost; crossing systems or proof surfaces does not automatically require decomposition.

Root preserves the user conversation and product judgment, and owns narrow authenticated integration and publication when publication is requested or inherently required: review the exact staged scope, run the risk-tier gate, commit, push, and verify CI/Pages or the public app. Active design and ordinary app iteration are local-first by default: build, test, and visually verify locally, but do not commit/push/deploy each intermediate change. Publish at a coherent checkpoint when Kavi says ship/publish/live, needs iPhone/public review, the work inherently changes live automation/data/auth, or the session needs a durable shared checkpoint. Do not create publish-only subagents or ask Kavi for a magic `PUSH X` phrase. Root must not repeat or absorb a large implementation, research, browser-proof, or database tranche that was deliberately delegated, but bounded review, integration, and publication are not such a tranche.

Give a worker a narrow, limited-history packet: objective, boundaries, relevant files, acceptance criteria, and required validation. Require a compact evidence-backed return covering decisions, files changed, checks run, failures, and remaining uncertainty rather than a narrated transcript. Retire workers promptly when their compartment ends or their context compacts; start fresh only when another substantial compartment warrants it. Keep meaningful decisions and unfinished substantial state in the repo so chat history is not the only memory. Do not update frontier/backlog files for trivial changes that create no durable project-state change.
