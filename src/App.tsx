  ...
  return <div className="wallet-home-command">
    <section className="wallet-hero-card">
      ...
      <div className="wallet-hero-actions">
        <button type="button" className="primary-show" onClick={() => openModal('SHOWABLE PROOF', 'Kavi badge QR', <ProofPreview kind="qr" code="[private-receipt-value-removed]" note="Representative QR surface. The real implementation should show the preserved pass/order QR with minimal chrome." />)}><NavIcon name="wallet" />Show Kavi QR</button>
      ...
      <div className="wallet-badge-fan" aria-label="Primary badge cards">
        <button type="button" className="mini-pass lotus-pass" onClick={() => openModal('BLACK LOTUS VIP', 'Kavi Grace', <ProofPreview kind="qr" code="[private-receipt-value-removed]" note="Badge proof belongs on Wallet Home because this is the thing most likely to be shown quickly." />)}>
          <span><EventKindIcon name="lotus" /></span>
          <strong>Kavi</strong>
          <small>Black Lotus</small>
        </button>
        <button type="button" className="mini-pass lotus-pass" onClick={() => openModal('BLACK LOTUS VIP', 'Chris Tom', <ProofPreview kind="qr" code="[private-receipt-value-removed]" note="Chris is represented here for owner-managed context; this is not multi-user account behavior." />)}>
          <span><EventKindIcon name="lotus" /></span>
          <strong>Chris</strong>
          <small>Black Lotus</small>
        </button>
        <button type="button" className="mini-pass premium-pass" onClick={() => openModal('PREMIUM WEEKEND', 'Juan Pereyra', <ProofPreview kind="qr" code="[private-receipt-value-removed]" note="Juan's badge proof remains handy from Home, with the full order receipt filed under Other." />)}>
    ...
    <section className="wallet-quick-grid" aria-label="Wallet quick checks">
      <button type="button" className="quick-proof black-lotus-proof" onClick={() => openModal('BADGE ORDER', 'Black Lotus order', <ProofPreview kind="receipt" note="Two Black Lotus VIP Early Bird badges. Total $2,025.26. QR key [private-receipt-value-removed]." />)}>
      ...
      <button type="button" className="quick-proof premium-proof" onClick={() => openModal('BADGE ORDER', 'Juan Premium Weekend', <ProofPreview kind="receipt" note="Premium Weekend Early Bird. Total $191.42. QR key [private-receipt-value-removed]." />)}>
  ...
  return <div className="wallet-layout">
    <section className="receipt-list" aria-label="Other wallet references">
      ...
      <article className="receipt-card">
        ...
        <div className="receipt-lines"><button type="button" onClick={() => openModal('BADGE RECEIPT', 'Kavi Black Lotus VIP', <ProofPreview kind="qr" code="[private-receipt-value-removed]" note="Badge proof is surfaced on Home; Other keeps the financial/original receipt context." />)}><span>Kavi Black Lotus VIP</span><b>Kavi</b></button><button type="button" onClick={() => openModal('BADGE RECEIPT', 'Chris Black Lotus VIP', <ProofPreview kind="qr" code="[private-receipt-value-removed]" note="Same Black Lotus order, assigned to Chris." />)}><span>Chris Black Lotus VIP</span><b>Chris</b></button><button type="button" onClick={() => openModal('SHIPPING', 'Black Lotus badge shipping', <p>Representative extracted line: shipping $15. The original order remains preserved under Show original.</p>)}><span>Shipping</span><b>$15</b></button></div>
        <div className="receipt-actions"><button type="button" onClick={() => openModal('ORIGINAL BLACK LOTUS ORDER', 'Black Lotus order confirmation', <ProofPreview kind="receipt" note="Two Black Lotus VIP Early Bird badges, total $2,025.26, transaction pi_3Tizh…" />)}>Show original</button><a className="button-link" href="[private-receipt-value-removed]" target="_blank" rel="noreferrer">Open Leap ↗</a></div>
      </article>
    </section>
    <aside className="wallet-show-card">
